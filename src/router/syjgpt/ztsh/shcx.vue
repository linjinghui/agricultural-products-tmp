  ...
  <div class="wrap">
    ...
    <div style="padding: 0 20px;" v-show="optionTab.acitve===0">
      <div class="wrap-form horiz" :class="{'show': formShow}">
        ...
        <div class="form-layer">
          <label class="star">审核状态:</label>
          <cmp-drop-menu class="f-dom" v-bind="optionShzt" v-model="optionShzt.result" @cbkClkItem="cbkClkShzt">
            <span slot="line" slot-scope="props">{{props.item.text}}</span>
          </cmp-drop-menu>
        </div>
        <div class="form-layer">
        ...
        <div class="form-layer">
          <label class="star">审核结束时间:</label>
          <cmp-date-picker class="f-dom" v-model="query._shjssjstart_"></cmp-date-picker>
        </div>
        <div class="form-layer" style="width: 185px;">
          <cmp-date-picker style="width: 100%;" class="f-dom" v-model="query._shjssjend_"></cmp-date-picker>
        </div>      
      ...
      <cmp-table v-bind="optionTabel">
        <tr slot="head">
          <td>主体名称</td><td>主体代码</td><td>经营场所</td><td>法定代表人</td><td>产业类型</td><td>主体性质</td><td>生产主体提交时间</td><td>审核状态</td><td>审核结束时间</td>
        </tr>
        <tr slot="body" slot-scope="props" @click="clkTabelItem(props.item)">
          <td>{{props.item._ztmc_}}</td><td>{{props.item._ztdm_}}</td><td>{{props.item._jycs_}}</td><td>{{props.item._fddbr_}}</td><td>{{props.item._cylx_}}</td><td>{{props.item._ztxz_}}</td><td>{{props.item._sczttjsj_}}</td><td>{{props.item._shzt_}}</td><td>{{props.item._shjssj_}}</td>
        </tr>
      </cmp-table>
    ...
    <div style="padding: 0 20px;" v-if="optionTab.acitve===1">
      <cmp-sh type="ck" @callback="clkRemoveSh"></cmp-sh>
    </div>
  </div>
</template>
...
<script>
  ...
  import {ajaxGetDshShcxData} from '../../../data/ajax.js';
  ...
  export default {
    ...
    data () {
      return {
        ...
        optionShzt: {
          placeholder: '请选择',
          show: true,
          data: [ {text: '全部', value: 1}, {text: '通过', value: 2}, {text: '不通过', value: 3} ],
          result: [0]
        },
        optionTabel: {
          data: []
    ...
    methods: {
      cbkTab: function (data) {
        this.optionTab.acitve = data.name === '查看' ? 1 : 0;
      },
      ...
      cbkClkShzt: function (data) {
        this.$set(this.query, '_shzt_', data);
      },
      clkRest: function () {
        this.query = {};
        this.clkSearch();
      ...
      callbackPagebar: function (data) {
        ...
        ajaxGetDshShcxData(Object.assign({
