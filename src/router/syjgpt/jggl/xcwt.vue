  ...
  <div class="wrap wtcz">
    ...
    <div style="padding: 0 20px;" v-show="optionTab.current.id===1">
      ...
      <cmp-table ref="rtable" v-bind="optionTabel">
        <tr slot="head">
          <td @click="clkOrder('title')">问题标题</td>
          <td @click="clkOrder('userName')">登记人</td>
          <td @click="clkOrder('sendTime')">登记时间</td>
        ...
        <tr slot="body" slot-scope="props" @click="clkView(props.item)">
          ...
          <td @click.stop="">
            ...
            <cmp-button class="theme" v-if="props.item.status===3" @click="clkBj(props.item)">办结</cmp-button>
          </td>
        </tr>
      </cmp-table>
