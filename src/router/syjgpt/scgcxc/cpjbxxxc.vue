    ...
    <div style="padding: 0 20px;">
      <div class="wrap-form horiz" :class="{'show': formShow}">
        <div class="form-layer">
          <label class="star">食品名称:</label>
          <cmp-input class="f-dom" v-model="query._spmc_" maxlength="50"></cmp-input>
        </div>
        <div class="form-layer">
          <label class="star">食品类型:</label>
          <cmp-drop-menu class="f-dom" v-bind="optionSplx" v-model="optionSplx.result" @cbkClkItem="cbkClkSplx">
      ...
      <cmp-table v-bind="optionTabel">
        <tr slot="head">
          <td>食品名称</td><td>食品代码</td><td>食品类型</td><td>产业类型</td><td>主体名称</td>
        </tr>
        <tr slot="body" slot-scope="props">
          <td>{{props.item._ztmc_}}</td><td>{{props.item._jycs_}}</td><td>{{props.item._fddbr_}}</td><td>{{props.item._cylx_}}</td><td>{{props.item._ztxz_}}</td>
        </tr>
      </cmp-table>
