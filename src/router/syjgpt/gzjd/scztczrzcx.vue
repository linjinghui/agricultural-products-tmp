  ...
  <div class="wrap scztczcx">
    ...
    <div style="padding: 0 20px;" v-show="optionTab.current.id===1">
      <div class="wrap-form search horiz" :class="{'show': formShow}">
        ...
        <div class="form-layer">
          <label class="star">执行的操作:</label>
          <cmp-input class="f-dom" v-model="query.operation" maxlength="50"></cmp-input>
        </div>
      ...
      <cmp-table ref="rtable" v-bind="optionTabel">
        <tr slot="head">
          <td @click="clkOrder('optTime')">操作时间</td>
          <td @click="clkOrder('entName')">主体名称</td>
          <td @click="clkOrder('optUserName')">用户名</td>
          <td @click="clkOrder('menuInfo')">操作菜单</td>
          <td @click="clkOrder('operation')">执行的操作</td>
        </tr>
        <tr slot="body" slot-scope="props">
          <td>{{utlFormatDate(props.item.optTime)}}</td>
          <td>{{props.item.entName}}</td>
          <td>{{props.item.optUserName}}</td>
          <td>{{props.item.menuInfo}}</td>
...
<script>
  import {Tab, Input, DropMenu, FlatDatePicker, Table, Button, PagebarPagesize} from 'web-base-ui';
  import {dataFormat} from 'web-js-tool';
  import {ajaxGetChildDivision, ajaxGetScztOptLogList} from '../../../data/ajax.js';
  ...
  export default {
    ...
    methods: {
      ...
      callbackPagebar: function (data) {
        ...
        ajaxGetScztOptLogList(Object.assign({
