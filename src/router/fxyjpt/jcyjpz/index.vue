<template>
    <div class="wrap jcyjpz" :class="{'max': maxContent}">
        <div class="p-left">
            <i class="fa fa-reorder" @click="maxContent=!maxContent"></i>
            <ul class="aside-nav">
                <li v-for="(item1,index) in navData2" :key="'lay1'+index">
...
<script>
  ...
  export default {
    ...
    mounted: function () {
      ...
      console.log(this.$root.isClickFxpz + '9929292929');

      if (this.navData.length > 0) {
        for (const nav of this.navData) {
          if (nav.permValue === 'stat:jcyj') {
            this.navData2 = [nav.children[0], nav.children[1]];
          }
        }

        var firstNav = this.navData2[0];

        if (!this.$root.isClickFxpz) {
          firstNav = this.navData2[1];
        }
